    ...
    plugins: [
      {
        ...
        enforce: 'pre',
        transform(code, id) {
          // Fix duplicate Backbone identifier in all @jupyter-widgets/base files
          if (id.includes('@jupyter-widgets/base')) {
            // Rename the Backbone import to BackboneLib to avoid conflicts
            let fixed = code.replace(
              /import \* as Backbone from ['"]backbone['"]/g,
              "import * as BackboneLib from 'backbone'"
            );
            // Replace all references to Backbone with BackboneLib
            // But preserve strings and comments
      ...
      },
      {
        ...
        enforce: 'post',
        transform(code, id) {
          // Inject Backbone.View polyfill at the top of any file that uses it
